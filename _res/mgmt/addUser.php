...
<form action="" method="post" id="addUser">
	...
	<div class="box">
		...
		PLEASE ONLY USE PASSWORDS GENERATED FROM <a href="https://www.random.org/strings/?num=10&len=15&digits=on&upperalpha=on&loweralpha=on&unique=on&format=html&rnd=new">HERE</a>, ACCOUNTS CAN NOT HAVE THEIR PASSWORDS CHANGED.
			...
			if( $_POST['submit'] ) {
				...
				try {
					...
					$dgroup   = $core->clean( $_POST['dgroup'] );
                                        $usertitle= $core->clean( $_POST['usertitle'] );
					...
					if( !$username or ( !$password and !$editid ) or !$dgroup or !$ugroups or !usertitle) {
						...
						if( $editid ) {
							...
							$db->query( "UPDATE users SET username = '{$username}'{$password}, email = '{$email}', habbo = '{$habbo}', displaygroup = '{$dgroup}', usergroups = '{$ugroups}',  usertitle = '{$usertitle}' WHERE id = '{$editid}'" );

						}
						else {
						
							$db->query( "INSERT INTO users VALUES (NULL, '{$username}', '{$password_enc}', '{$email}', '{$habbo}', '{$dgroup}', '{$ugroups}', '{$usertitle}');" );
